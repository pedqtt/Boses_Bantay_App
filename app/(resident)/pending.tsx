  ...
  return (
    <SafeAreaView className="flex-1">
      ...
      <View className="flex-1 px-8 justify-center items-center">
        ...
        <Text className="text-[26px] font-semibold text-ink tracking-tight mb-3 text-center">
          Account Under Review
        </Text>

        <Text className="text-[17px] text-ink-soft mb-2 text-center leading-7">
          Thank you for uploading your Barangay ID, {profile?.firstName || "Resident"}.
        </Text>

        <Text className="text-[17px] text-ink-soft mb-10 text-center leading-7">
        ...
        <Pressable
          ...
          disabled={loading}
          className="bg-brand w-full rounded-2xl py-4 items-center mb-4 overflow-hidden active:opacity-85"
        >
          {loading ? (
            <ActivityIndicator color="white" />
          ) : (
            <Text className="text-white font-semibold text-[18px]">Check Approval Status</Text>
          )}
        </Pressable>

        <Pressable onPress={handleSignOut} className="py-4">
          <Text className="text-ink-soft font-medium text-[17px]">Log out for now</Text>
        </Pressable>
      </View>
    </ScreenBackground>
    </SafeAreaView>
